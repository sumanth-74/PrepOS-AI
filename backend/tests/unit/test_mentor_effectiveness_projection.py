"""Integration tests for mentor effectiveness twin projection."""
...
from prepos.application.twin.projection_ports import MentorEffectivenessSummary
...
@pytest.mark.asyncio
async def test_mentor_effectiveness_projection_persists_payload() -> None:
    tenant_id = uuid4()
    student_id = uuid4()
    exam_id = "neet"
    ...
    mentor_effectiveness_port.get_mentor_effectiveness_summary = AsyncMock(
        return_value=MentorEffectivenessSummary(
    ...
    builder = TwinProjectionBuilder(
        ...
        mentor_effectiveness_port=mentor_effectiveness_port,
    ...
    effectiveness = mentor_payload["mentor_effectiveness"]
    assert effectiveness["best_action"] == "CONTACT_STUDENT"
    assert effectiveness["effectiveness_score"] == 84.2
    assert effectiveness["sample_size"] == 42
