...
@pytest.mark.asyncio
async def test_dashboard_exposes_outcome_and_optimization_metrics() -> None:
    ...
    twin = PreparationTwin(
        ...
        twin_payload={
            "intervention_effectiveness": {
                "last_effectiveness_score": 82.4,
                "outcome_status": "HIGHLY_EFFECTIVE",
            },
            "optimization": {
                "best_intervention": "REVISION_SPRINT",
                "historical_effectiveness": 78.2,
                "optimized_intervention_score": 84.0,
            },
        },
        generated_at=now,
    ...
    assert dashboard.best_intervention == "REVISION_SPRINT"
    assert dashboard.historical_effectiveness == Decimal("78.2")
    assert dashboard.last_effectiveness_score == Decimal("82.4")
    assert isinstance(twin_response, TwinResponse)
    assert twin_response.intervention_effectiveness is not None
    assert twin_response.optimization is not None
