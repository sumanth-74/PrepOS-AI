...
async def _publish_twin_decision(envelope: DomainEventEnvelope) -> None:
    ...
    async with session_scope() as session:
        ...
        service = build_twin_decision_service(session=session, read_service=read_service)
        await service.publish_twin_decision_updated(
...
async def on_goal_updated_twin_decision(envelope: DomainEventEnvelope) -> None:
    await _publish_twin_decision(envelope)


async def on_forecast_updated_twin_decision(envelope: DomainEventEnvelope) -> None:
    await _publish_twin_decision(envelope)


async def on_milestone_updated_twin_decision(envelope: DomainEventEnvelope) -> None:
    await _publish_twin_decision(envelope)


async def on_learning_graph_updated_twin_decision(envelope: DomainEventEnvelope) -> None:
    await _publish_twin_decision(envelope)


async def on_study_behavior_updated_twin_decision(envelope: DomainEventEnvelope) -> None:
    await _publish_twin_decision(envelope)


async def on_twin_decision_updated_projection(envelope: DomainEventEnvelope) -> None:
    if envelope.tenant_id is None:
        return
    payload = envelope.payload
    async with session_scope() as session:
...
def register_event_handlers() -> None:
    dispatcher.register(
        "GoalUpdated",
        "twin_decision_engine",
        on_goal_updated_twin_decision,
    )
    dispatcher.register(
        "ForecastUpdated",
        "twin_decision_engine",
        on_forecast_updated_twin_decision,
    )
    dispatcher.register(
        "MilestoneUpdated",
        "twin_decision_engine",
        on_milestone_updated_twin_decision,
    )
    dispatcher.register(
        "LearningGraphUpdated",
        "twin_decision_engine",
        on_learning_graph_updated_twin_decision,
    )
    dispatcher.register(
        "StudyBehaviorUpdated",
        "twin_decision_engine",
        on_study_behavior_updated_twin_decision,
    )
    dispatcher.register(
        "TwinDecisionUpdated",
        "twin_rebuild_orchestrator",
        on_twin_decision_updated_projection,
    )
