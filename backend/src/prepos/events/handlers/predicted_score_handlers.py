...
async def _publish_predicted_score(envelope: DomainEventEnvelope) -> None:
    ...
    async with session_scope() as session:
        ...
        service = build_predicted_score_service(session=session, read_service=read_service)
...
async def on_learning_graph_updated_predicted_score(envelope: DomainEventEnvelope) -> None:
    await _publish_predicted_score(envelope)


async def on_forecast_updated_predicted_score(envelope: DomainEventEnvelope) -> None:
    await _publish_predicted_score(envelope)


async def on_twin_recommendations_updated_predicted_score(envelope: DomainEventEnvelope) -> None:
    await _publish_predicted_score(envelope)


async def on_predicted_score_updated_twin_projection(envelope: DomainEventEnvelope) -> None:
    if envelope.tenant_id is None:
        return
    payload = envelope.payload
    async with session_scope() as session:
        await request_twin_incremental_update(
            ...
            section=TwinProjectionSection.PREDICTED_SCORE,
            correlation_id=envelope.correlation_id,
            causation_id=str(envelope.event_id),
        )
...
def register_event_handlers() -> None:
    dispatcher.register(
        ...
        "predicted_score_engine",
        on_learning_graph_updated_predicted_score,
    )
    dispatcher.register(
        "ForecastUpdated",
        "predicted_score_engine",
        on_forecast_updated_predicted_score,
    )
    dispatcher.register(
        "TwinRecommendationsUpdated",
        "predicted_score_engine",
        on_twin_recommendations_updated_predicted_score,
    )
    dispatcher.register(
        "PredictedScoreUpdated",
        "twin_rebuild_orchestrator",
        on_predicted_score_updated_twin_projection,
    )
