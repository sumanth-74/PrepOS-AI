...
async def _publish_milestones(envelope: DomainEventEnvelope) -> None:
    ...
    async with session_scope() as session:
        ...
        service = build_milestone_service(session=session, read_service=read_service)
...
async def on_goal_updated_milestones(envelope: DomainEventEnvelope) -> None:
    await _publish_milestones(envelope)


async def on_forecast_updated_milestones(envelope: DomainEventEnvelope) -> None:
    await _publish_milestones(envelope)


async def on_learning_graph_updated_milestones(envelope: DomainEventEnvelope) -> None:
    await _publish_milestones(envelope)


async def on_milestone_updated_twin_projection(envelope: DomainEventEnvelope) -> None:
    if envelope.tenant_id is None:
        return
    payload = envelope.payload
    async with session_scope() as session:
        await request_twin_incremental_update(
            ...
            section=TwinProjectionSection.MILESTONES,
            correlation_id=envelope.correlation_id,
            causation_id=str(envelope.event_id),
        )
...
def register_event_handlers() -> None:
    dispatcher.register(
        "GoalUpdated",
        "milestone_engine",
        on_goal_updated_milestones,
    )
    dispatcher.register(
        "ForecastUpdated",
        "milestone_engine",
        on_forecast_updated_milestones,
    )
    dispatcher.register(
        "LearningGraphUpdated",
        "milestone_engine",
        on_learning_graph_updated_milestones,
    )
    dispatcher.register(
        "MilestoneUpdated",
        "twin_rebuild_orchestrator",
        on_milestone_updated_twin_projection,
    )
