...
from prepos.application.twin.rebuild_factory import (
    build_mentor_effectiveness_learning_service,
...
async def _recompute_effectiveness(envelope: DomainEventEnvelope) -> None:
    ...
    async with session_scope() as session:
        service = build_mentor_effectiveness_learning_service(session=session)
        ...
        logger.info(
            "mentor_effectiveness_recomputed",
...
async def on_mentor_case_resolved_effectiveness(envelope: DomainEventEnvelope) -> None:
    await _recompute_effectiveness(envelope)


async def on_intervention_outcome_calculated_effectiveness(envelope: DomainEventEnvelope) -> None:
    await _recompute_effectiveness(envelope)


async def on_mentor_effectiveness_updated_projection(envelope: DomainEventEnvelope) -> None:
    if envelope.tenant_id is None:
        return
    payload = envelope.payload
    exam_id = payload.get("exam_id")
    ...
    async with session_scope() as session:
        await request_twin_incremental_update(
            ...
            student_id=UUID(str(student_id)),
            exam_id=str(exam_id),
            section=TwinProjectionSection.MENTOR_EFFECTIVENESS,
            correlation_id=envelope.correlation_id,
            causation_id=str(envelope.event_id),
        )
...
def register_event_handlers() -> None:
    dispatcher.register(
        "MentorCaseResolved",
        "mentor_effectiveness_engine",
        on_mentor_case_resolved_effectiveness,
    )
    dispatcher.register(
        "InterventionOutcomeCalculated",
        "mentor_effectiveness_engine",
        on_intervention_outcome_calculated_effectiveness,
    )
    dispatcher.register(
        "MentorEffectivenessUpdated",
        "twin_rebuild_orchestrator",
        on_mentor_effectiveness_updated_projection,
    )
